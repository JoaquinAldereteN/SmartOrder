  ...
  return (
    <div className="relative w-full" ref={ref}>
      <button
        ...
        className="w-full text-left bg-gray-700 text-white border border-gray-600 px-3 py-2 rounded focus:outline-none flex justify-between items-center"
      >
      ...
      {open && (
        <ul className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded bg-gray-700 border border-gray-600 shadow-lg">
          {options.map((opt) => (
            <li
              key={opt.value}
              onClick={() => {
              ...
              className={`cursor-pointer px-3 py-2 text-white hover:bg-gray-600 ${
                value === opt.value ? 'font-semibold bg-gray-600' : ''
              }`}
            >
              {opt.label}
            </li>
